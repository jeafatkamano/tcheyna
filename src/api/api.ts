...
export const otpAPI = {
  send: () =>
    request<{ message: string; phone: string }>("/otp/send", { method: "POST" }),

  verify: (code: string) =>
    request<{ message: string; trust_level: number; badge: string }>("/otp/verify", {
      method: "POST",
      body: JSON.stringify({ code }),
    }),

  resend: () =>
    request<{ message: string; phone: string }>("/otp/resend", { method: "POST" }),
};
